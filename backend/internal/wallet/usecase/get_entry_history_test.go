...
func TestGetEntryHistory(t *testing.T) {
	tests := []struct {
		name        string
		walletID    int64
		limit       int32
		...
		{
			name:     "Success - return entry history",
			walletID: 1,
			limit:    10,
			offset:   0,
			setupMocks: func(repo *MockWalletRepository) {
				repo.On("GetWalletByID", mock.Anything, int64(1)).Return(&domain.Wallet{ID: 1, UserID: 7, Balance: 1000, Currency: "VND"}, nil)
				repo.On("GetListEntries", mock.Anything, int64(1), int32(10), int32(0)).Return([]domain.Entry{
					{ID: 1, WalletID: 1, Amount: -500},
					{ID: 2, WalletID: 1, Amount: 200},
				}, nil)
			},
			expectedErr: "",
			expectedLen: 2,
		...
		{
			name:     "Failure - list entry error",
			walletID: 3,
			limit:    5,
			offset:   1,
			setupMocks: func(repo *MockWalletRepository) {
				repo.On("GetWalletByID", mock.Anything, int64(3)).Return(&domain.Wallet{ID: 3, UserID: 9, Balance: 500, Currency: "USD"}, nil)
				repo.On("GetListEntries", mock.Anything, int64(3), int32(5), int32(1)).Return(nil, errors.New("db error"))
			},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			entries, err := uc.GetEntryHistory(context.Background(), tc.walletID, tc.limit, tc.offset)
